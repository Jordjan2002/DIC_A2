        ...
        while not all(dones):
            for idx, (agent, env, ren) in enumerate(zip(all_agents, all_envs, renderers)):
                if dones[idx]:
                    continue
                with torch.no_grad():
                    observations[idx]['image'] = torch.FloatTensor(observations[idx]['image']).unsqueeze(0).to(agent.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--agents-per-config", type=int, default=1)
    ...
    env_config = EnvConfig(max_steps=20)
